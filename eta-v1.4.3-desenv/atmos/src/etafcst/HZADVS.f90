...
!> SUBROUTINE HZADVS
!>
!> SUBPROGRAM: HZADVS - HORIZONTAL ADVECTION
!> PROGRAMMER: JANJIC
!> ORG: W/NP22
!> DATE: 93-10-28
!>
...
!
    IMPLICIT NONE
!
    REAL   (KIND=R4KIND), PARAMETER :: TLC = 2. * 0.703972477
!
#include "sp.h"
!
    INTEGER(KIND=R4KIND), PARAMETER :: IM1  = IM - 1
    INTEGER(KIND=R4KIND), PARAMETER :: IMJM = IM * JM - JM / 2
    INTEGER(KIND=R4KIND), PARAMETER :: JAMD = (JAM * 2 - 10) * 3
!
    LOGICAL(KIND=L4KIND)                                                                        ::&
    & ITER2
!
...
!    
        DO 50 I=IVL,IVH
            IF (I >= MY_IS_GLB-ILPAD2 .AND. I <= MY_IE_GLB+IRPAD2 .AND. J >= MY_JS_GLB-JBPAD2     &
    &                                 .AND. J <= MY_JE_GLB+JTPAD2) THEN
!
                JAKTWO        = JAKTWO + 1
                KVHAS(JAKTWO) = JA
                IVLAS(JAKTWO) = I
...
!
                IF (K < LM) THEN
                    RDPDXS(I,J) = 1.0 / (ADPDX(I,J) * (DETA(K) + 0.5 * DETA(K+1)) / DETA(K))
                    RDPDYS(I,J) = 1.0 / (ADPDY(I,J) * (DETA(K) + 0.5 * DETA(K+1)) / DETA(K))
                END IF
    120 END DO
!---------------------------------------------------------- 
...
!
                            TST(IX + IHE(JX) + IFP, JX + ISP) = 0.5 * (TST(IX, JX)                &
...
!                   
                        ...
                        F3 = ARRAY3(JAK)
!
                        ADT(IX,JX) = F0 * TST(IX,JX)                                              &
    &                              + F1 * TST(IX + IHE(JX) + IFP, JX + ISP      )                 &
...
!
    ...
    &                        + F2 * Q2M(IX+IHE(JX)+IFQ,JX+ISQ) + F3 * Q2M(IX+IPQ,JX+ISP+ISQ)
!
                ADQ2L(IX,JX) = F0 * Q2L(IX,JX) + F1 * Q2L(IX+IHE(JX)+IFP,JX+ISP)                  &
    &                        + F2 * Q2L(IX+IHE(JX)+IFQ,JX+ISQ) + F3 * Q2L(IX+IPQ,JX+ISP+ISQ)
            175 END DO
!        
        END IF
!--------------------------------------------- 
...
!---------------------------------------------
        DO 131 J=MYJS1_P4,MYJE1_P4
            DO 131 I=MYIS_P4,MYIE_P4
                UDY(I,J) = UST(I,J) * DY
                FEW(I,J) = UDY(I,J) * ADPDX(I,J)
...
! THE NE AND SE FLUXES ARE ON H POINTS (ACTUALLY JUST TO THE NE AND SE OF EACH H POINT)
!--------------------------------------------------------------------------------------
        DO 146 J=MYJS2_P4,MYJE2_P4
            DO 146 I=MYIS_P4,MYIE_P4
                TEMPA(I,J) = UDY(I,J) + VDX(I,J)
                TEMPB(I,J) = UDY(I,J) - VDX(I,J)
    146 END DO
!    
        DO 151 J=MYJS2_P4,MYJE2_P4
            DO 151 I=MYIS_P4,MYIE_P4
                FNE(I,J) = (TEMPA(I+IHE(J),J) + TEMPA(I,J+1)) * (DPDE(I,J) + DPDE(I+IHE(J),J+1))
                FSE(I,J) = (TEMPB(I+IHE(J),J) + TEMPB(I,J-1)) * (DPDE(I,J) + DPDE(I+IHE(J),J-1))
    151 END DO
!-----------------------------------------------
...
!
                IF ((VTMS(I+IHE(J),J,K)+VTMS(I+IHW(J),J,K)) == 1) THEN
                    UEW(I,J) = UEW(I,J) * 0.5
                END IF
!
                UNS(I,J) = (FNS(I+IHW(J),J) + FNS(I+IHE(J),J)) * (UST(I,J+1) - UST(I,J-1))
!
                IF ((VTMS(I,J+1,K)+VTMS(I,J-1,K)) == 1) THEN
                    UNS(I,J) = UNS(I,J) * 0.5
                END IF
!
                VEW(I,J) = (FEW(I,J-1) + FEW(I,J+1)) * (VST(I+IHE(J),J) - VST(I+IHW(J),J))
!
                IF ((VTMS(I+IHE(J),J,K)+VTMS(I+IHW(J),J,K)) == 1) THEN
                    VEW(I,J) = VEW(I,J) * 0.5
                END IF
...
!----------------------------------------------------
                UNE(I,J) = (FNE(I+IVW(J),J) + FNE(I+IVE(J),J)) * (UST(I+IVE(J),J+1) - UST(I,J))
                IF ((VTMS(I+IVE(J),J+1,K)+VTMS(I,J,K)) == 1) THEN
                    UNE(I,J) = UNE(I,J) * 0.5
                END IF
!
                USE(I,J) = (FSE(I+IVW(J),J) + FSE(I+IVE(J),J)) * (UST(I+IVE(J),J-1) - UST(I,J))
                IF ((VTMS(I+IVE(J),J-1,K)+VTMS(I,J,K)) == 1) THEN
                    USE(I,J) = USE(I,J) * 0.5
                END IF
!
...
!---------------------------- 
        DO 200 J=MYJS5_P2,MYJE5_P2
            DO 200 I=MYIS_P2,MYIE_P2
                IF ( .NOT. (K < LM .AND. ITER2 .AND. (VTMS(I,J,K+1) == 1))) THEN
...
!
    ...
    &                        +    UNE(I+IVW(J),J-1) +  UNE(I       ,J  )                          &
    &                        +    USE(I       ,J  ) +  USE(I+IVW(J),J+1)                          &
    &                        +   UEWS(I+IVW(J),J  ) + UEWS(I+IVE(J),J  )                          &
    &                        +   UNSS(I       ,J-1) + UNSS(I       ,J+1)                          &
    &                        +   UNES(I+IVW(J),J-1) + UNES(I       ,J  )                          &
    &                        +   USES(I       ,J  ) + USES(I+IVW(J),J+1))                         &
    &                        * RDPDXS(I       ,J  ) *  FAD(I+IVW(J),J  )
!
                    ADV(I,J) =  (VEW(I+IVW(J),J  ) +  VEW(I+IVE(J),J  )                           &
    &                        +   VNS(I       ,J-1) +  VNS(I       ,J+1)                           &
    &                        +   VNE(I+IVW(J),J-1) +  VNE(I       ,J  )                           &
    &                        +  VSE(I        ,J  ) +  VSE(I+IVW(J),J+1)                           &
    &                        +  VEWS(I+IVW(J),J  ) + VEWS(I+IVE(J),J  )                           &
    &                        +  VNSS(I       ,J-1) + VNSS(I       ,J+1)                           &
    &                        +  VNES(I+IVW(J),J-1) + VNES(I       ,J  )                           &
    &                        +  VSES(I       ,J  ) + VSES(I+IVW(J),J+1))                          &
    &                        * RDPDYS(I      ,J  ) *  FAD(I+IVW(J),J  )
                END IF
    200 END DO
!
        IF (ITER2) THEN
...
!
    ...
    &                      + F2 * UST(IX+IVE(JX)+IFQ,JX+ISQ) + F3 * UST(IX+IPQ,JX+ISP+ISQ)
!
                ADV(IX,JX) = F0 * VST(IX,JX) + F1 * VST(IX+IVE(JX)+IFP,JX+ISP)                    &
    &                      + F2 * VST(IX+IVE(JX)+IFQ,JX+ISQ) + F3 * VST(IX+IPQ,JX+ISP+ISQ)
        205 END DO
        END IF
!----------------------------
! END OF THIS UPSTREAM REGION
...
!    
        DO 250 J=MYJS2,MYJE2
            DO 250 I=MYIS1,MYIE1
                U(I,J,K) = (ADU(I,J) * (2.0 * VM(I,J)) + U(I,J,K) ) * VTM(I,J,K)
                V(I,J,K) = (ADV(I,J) * (2.0 * VM(I,J)) + V(I,J,K) ) * VTM(I,J,K)
    250 END DO
!
        IF (K == LM) THEN
