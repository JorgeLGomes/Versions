...
!
    INTEGER(KIND=I4KIND), DIMENSION(JAMD)                                                       ::&
...
!------------------------
    INTEGER(KIND=I4KIND)                                                                        ::&
    & IEND    , I       , J       , K       , JAKONE  , JA      , IHL     , IHH     , JAKTOW  ,   &
    & IVL     , IVH     , JAK     , IX      , JX      , ISP     , ISQ     , IFP     , IPQ     ,   &
    & JAKTWO  , IFQ
!
    REAL   (KIND=R4KIND)                                                                        ::&
...
!                        
                        ELSE IF                                                                   &
    &                       (HTM(IX+IHE(JX)+IFP,JX+ISP,K) + HTM(IX+IPQ,JX+ISP+ISQ,K) < 0.99) THEN 
!
                             TST(IX+IHE(JX)+IFP,JX+ISP    ) =  TST(IX            ,JX    )
                             TST(IX+IPQ        ,JX+ISP+ISQ) =  TST(IX+IHE(JX)+IFQ,JX+ISQ)
!
                        ELSE IF                                                                   &
    &                       (HTM(IX+IHE(JX)+IFQ,JX+ISQ,K) + HTM(IX+IPQ,JX+ISP+ISQ,K) < 0.99) THEN
!
                             TST(IX+IHE(JX)+IFQ,JX+ISQ    ) = TST(IX            ,JX    )
                             TST(IX+IPQ        ,JX+ISP+ISQ) = TST(IX+IHE(JX)+IFP,JX+ISP)
!
                        ELSE IF                                                                   &
                            (HTM(IX+IHE(JX)+IFP,JX+ISP,K)                                         &
    &                                                     +                                       &
...
!---------------------------- 
    ...
    &                    +  UNE(I+IVW(J),J-1) + UNE(I       ,J) + USE(I,J  ) + USE(I+IVW(J),J+1)) &
    &                    *  RDPDX(I     ,J  ) * FAD(I+IVW(J),J)
!
                ADV(I,J) = (VEW(I+IVW(J),J  ) + VEW(I+IVE(J),J) + VNS(I,J-1) + VNS(I       ,J+1)  &
    &                    +  VNE(I+IVW(J),J-1) + VNE(I       ,J) + VSE(I,J  ) + VSE(I+IVW(J),J+1)) &
        ...
        ELSE
            DO 270 J=MYJS2,MYJE2
                DO 270 I=MYIS1,MYIE1
                    ADQ2HL(I,J,K  ) = ADQ2L(I,J)
                        Q2(I,J,K-1) = ADQ2M(I,J) * HM(I,J) + Q2(I,J,K-1)
        270 END DO
        END IF
!
500 END DO
...
!
    DO 600 K=2,LM
        DO J=MYJS2,MYJE2
            DO I=MYIS1,MYIE1
                HM(I,J) = HTM(I,J,K) * HBM2(I,J)
                Q2(I,J,K-1) = ADQ2HL(I,J,K-1) * HM(I,J) + Q2(I,J,K-1)
            END DO
        END DO
600 END DO
!
