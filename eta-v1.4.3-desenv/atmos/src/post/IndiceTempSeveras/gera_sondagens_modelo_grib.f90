...
!       integer, parameter   :: Max_Sond_Levs=200
  ...
  ! Parametros do Modelo Passados via namelist
       ...
       real                 :: loni1	      ! Longitude a oeste 
       real                 :: lati1	      ! Latitude ao sul
...
! Variáveis de entrada
       ...
       namelist/model_grids/lm,loni,lati,res,FreqModOut,        &
     &                     idia,imes,iano,ihora,ARQIN,ARQOUT,strm_opt 
       ...
       allocate(mlevs(lm))
       open(1,file='index_mlevs.txt',form='formatted',status='old')
       do k=1,lm
        read(1,*)mlevs(k)
       enddo
       close(1)       
...
!        Loop pelo número de níveis do ponto de grade k.
       ...
       enddo
299  format (2(1x,I3),3(1x,f7.3),1x,I2,1x,I3,1x,I2)
