...
c
      integer nx,ny,nz,i,j,k,l,n,ng,it,ip,jp,nsfcfld
     .         ,iyear,imonth,iday,ifcsthr,n1,n2
     .	       ,datsav,ii,jj
      integer kgds(200)
...
c
      ...
      character(LEN=255):: outdir,outfile,outfile2,gribfile
      character(LEN=255):: gdsfile
      character(LEN=2)  :: gproj
      character(LEN=12) ::   atime
      character(LEN=6)  ::  model,teste2
...
c
      ...
      n2=index(gdsfile,' ')-1
      open(unit=14,file=gdsfile(1:n2),form='unformatted',
     +             access='sequential')
...
C
	...
	write(6,*) 'GDS written to ', gdsfile(1:n2)
...
c
     ...
     .        ,j,k,nx,IRETO,KNUM,IRET1,IRET0
c
      character*(*) etafile
      character*(4) etafile2
...
C	call getgb(11,0,nxny,0,JPDS,JGDS,nwords,KNUM,KPDS,KGDS,
C     &     BITMAP,slp(1,1),IRET1)

C	write(6,*) 'first GETGB, IRET1= ', IRET1
...
C############# ZSFC ##############################
	...
	jpds(7)=0
       ...
       call getgb(12,0,nxny,0,JPDS,JGDS,nwords,KNUM,KPDS,KGDS,
