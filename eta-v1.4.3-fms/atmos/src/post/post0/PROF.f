...
C------------------------------------------------------------------------
                             C H A R A C T E R
     & RSTFIL*80,RESTHR*4,LABEL*32,CISTAT*8,CIDSTN(NSTAT)*8
     &,FNAME*80,ENVAR*80,BLANK*4
C------------------------------------------------------------------------
      DATA BLANK/'    '/
C------------------------------------------------------------------------
C***
...
C	write(6,*) 'NFCST,NBC,LIST,DT ', NFCST,NBC,LIST,DT
...
C	write(6,*) 'RUN,IDAT,IHRST,NTSD:', RUN,IDAT,IHRST,NTSD
      READ(LRSTRT)
C
      DO L=1,LM
...
C
      ...
      ENDDO
C
      READ(LRSTRT)RUN,IDAT,IHRST,NTSD,LABEL,itag
...
C
      ...
      ENDDO
C
CJLG01      READ(LRSTRT)(((DUM(I,J,N),I=1,IM),J=1,JM),N=1,4)
...
C
   ...
   80     FORMAT('RESTRT',I6.6)
        ELSE
          WRITE(RSTFIL,85)ITAG0,RESTHR
   85     FORMAT('RESTRT',I6.6,'.',a4)
        ENDIF
C
        KRST = INDEX(RSTFIL,' ') -1
        IF(KRST.LE.0) KRST = LEN(RSTFIL)
...
C
      ...
      PRODAT(NWORD13+41) = SMC   (N,5)
      PRODAT(NWORD13+42) = SMC   (N,6)
      PRODAT(NWORD13+43) = SMC   (N,7)
      PRODAT(NWORD13+44) = SMC   (N,8)
      PRODAT(NWORD13+45) = STC   (N,1)
      PRODAT(NWORD13+46) = STC   (N,2)
      PRODAT(NWORD13+47) = STC   (N,3)
      PRODAT(NWORD13+48) = STC   (N,4)
      PRODAT(NWORD13+49) = STC   (N,5)
      PRODAT(NWORD13+50) = STC   (N,6)
      PRODAT(NWORD13+51) = STC   (N,7)
      PRODAT(NWORD13+52) = STC   (N,8)
      PRODAT(NWORD13+53) = SM    (N) + SICE(N)
      PRODAT(NWORD13+54) = CFRACL(N)*100.
      PRODAT(NWORD13+55) = CFRACM(N)*100.
      PRODAT(NWORD13+56) = CFRACH(N)*100.
      PRODAT(NWORD13+57) = SR    (N)*100.
      PRODAT(NWORD13+58) = NINT(HBOT(N))
C
