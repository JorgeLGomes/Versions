...
!
      ...
      REAL  :: ARDSW,ARDLW,ASRFC,AVRAIN,AVCNVC, RRNUMSW, RRNUMLW
!
!     ##################################################################
!
      INCLUDE "parmeta"
...
!
      INTEGER, PARAMETER ::                                            &                                            
      NSTAT=2000,LCL1ML=15,LCL1SL=58,NWORDM=(LCL1ML+1)*LM+2*LCL1SL,    &
...
!
      print*,NUMSTA
      DO N=1,NUMSTA
      print*,NUMSTA,N,IHINDX(N),JHINDX(N)
       HBM2(N)=DUM(IHINDX(N),JHINDX(N),1)
      ENDDO
      print*,"passei read CNST"
!
      READ(NHB)                 !VBM2
      READ(NHB)                 !VBM3
      READ(NHB) DUM(:,:,1)      !SM
...
!
       DO N=1,NUMSTA
        ASWIN(N)=DUM(IHINDX(N),JHINDX(N),1)*RRNUMSW
        ASWOUT(N)=DUM(IHINDX(N),JHINDX(N),2)*RRNUMLW
        ASWTOA(N)=DUM(IHINDX(N),JHINDX(N),3)*RRNUMSW
        ALWIN(N)=DUM(IHINDX(N),JHINDX(N),4)*RRNUMLW
        ALWOUT(N)=DUM(IHINDX(N),JHINDX(N),5)*RRNUMLW
        ALWTOA(N)=DUM(IHINDX(N),JHINDX(N),6)*RRNUMLW
       ENDDO         
!
       READ(LRSTRT) DUM(:,:,1:6) 
!
...
!
       ...
       PRODAT(NWORD15+41)=SMC(N,5)
       PRODAT(NWORD15+42)=SMC(N,6)
       PRODAT(NWORD15+43)=SMC(N,7)
       PRODAT(NWORD15+44)=SMC(N,8)
       PRODAT(NWORD15+45)=STC(N,1)
       PRODAT(NWORD15+46)=STC(N,2)
       PRODAT(NWORD15+47)=STC(N,3)
       PRODAT(NWORD15+48)=STC(N,4)
       PRODAT(NWORD15+49)=STC(N,5)
       PRODAT(NWORD15+50)=STC(N,6)
       PRODAT(NWORD15+51)=STC(N,7)
       PRODAT(NWORD15+52)=STC(N,8)
       PRODAT(NWORD15+53)=SM(N)+SICE(N)
       PRODAT(NWORD15+54)=CFRACL(N)*100.
       PRODAT(NWORD15+55)=CFRACM(N)*100.
       PRODAT(NWORD15+56)=CFRACH(N)*100.
       PRODAT(NWORD15+57)=SR(N)*100.
       PRODAT(NWORD15+58)=NINT(HBOT(N))
!
