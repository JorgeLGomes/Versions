...
!
    ...
    REAL   (KIND=R4KIND), PARAMETER :: Q2INI  =    0.50 
    REAL   (KIND=R4KIND), PARAMETER :: EPSQ2  =    0.12
    REAL   (KIND=R4KIND), PARAMETER :: EPSWET =    0.0 
    REAL   (KIND=R4KIND), PARAMETER :: Z0LAND =    0.10
    REAL   (KIND=R4KIND), PARAMETER :: Z0SEA  =    0.001
    REAL   (KIND=R4KIND), PARAMETER :: FCM    =    0.00001 
    REAL   (KIND=R4KIND), PARAMETER :: DTR    =    0.1745329E-1
    REAL   (KIND=R4KIND), PARAMETER :: A1     =  610.78
    REAL   (KIND=R4KIND), PARAMETER :: WA     =     0.10
...
!
    ...
    & RSIN1   , RCOS1   , RCOS2   , ULM     , VLM     , TLM     , QLM     , PLM     , APELM   ,   &
...
!------------------------------------
    ...
    IF (LSST) THEN
        IF (LSSTMNTHLY) THEN
           CALL FLDS_UPDATE_DRIVER('READ','SST')
...
!--------------------------------------------------------------------------------------------------
! IF NOT RUNNING THE MODEL, PRINT DATE OF INITIAL CONDITIONS JUST READ AND STOP. 
! OTHERWISE, CONTINUE.
!--------------------------------------------------------------------------------------------------
    IF (RUN) GOTO 190
!
...
!--------
    IF (MYPE == 0) THEN 
        WRITE(0,*) "JLG TESTE"
        WRITE(0,*) IDAT(3), IDAT(1), IDAT(2), IHRST
        WRITE(CYR ,'(I4)') IDAT(3)
        WRITE(CMON,'(I2)') IDAT(1)
! 
        IF (IDAT(1) < 10) CMON(1:1) = '0'
!
        WRITE(CDAY,'(I2)') IDAT(2)
!
        IF (IDAT(2) < 10) CDAY(1:1) = '0'
!
        WRITE(CUTC,'(I2)') IHRST
! 
        IF (IHRST < 10)   CUTC(1:1) = '0'
!
        WRITE(CHARDATE,'(A4,3A2)') CYR, CMON, CDAY, CUTC
        WRITE(0,*) CHARDATE
!	
    END IF
!-------------------------- 
        ...
        DO J=JS_LOC_TABLE(MYPE),JE_LOC_TABLE(MYPE)
            DO I=IS_LOC_TABLE(MYPE),IE_LOC_TABLE(MYPE)
                ...
                CLDEFI(I,J) =   1.
                  ...
                  HBOT(I,J) =   0.
!--------------------------------------------------------------------------------------------------------
! AT THIS POINT, WE MUST CALCULATE THE INITIAL POTENTIAL TEMPERATURE OF THE SURFACE AND OF THE SUBGROUND.
...
! SET TKE TO ZERO IN THE THE LOWEST MODEL LAYER.  IN THE LOWEST TWO ATMOSPHERIC ETA LAYERS SET TKE TO A SMALL VALUE (Q2INI).
!---------------------------------------------------------------------------------------------------------------------------    
        DO K=1,LM1
            DO J=JS_LOC_TABLE(MYPE),JE_LOC_TABLE(MYPE)
                DO I=IS_LOC_TABLE(MYPE),IE_LOC_TABLE(MYPE)
...
! THE ARRAY 'LVL' WILL COORDINATE VERTICAL LOCATIONS BETWEEN THE LIFTED WORKING ARRAYS AND THE FUNDAMENTAL MODEL ARRAYS.
